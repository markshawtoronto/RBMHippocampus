...
%     postspikes = spalloc(1, round(simdur/dt)+1, 20*simdur); % expect firing at, say, 20 Hz
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    basePhi = 0+2*col; % baseline oscillator phase variable
    naming = basePhi;
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    VCOPhi = zeros(1,nVCOs); % VCO phase variable(s)
    prefHDs = [0 2*pi/3 4*pi/3]; %% This is easier: 3 preferred directions (radians)
...
%% Type 2 plotting
    ...
    if nVCOs==1
      ...
      abstractThr = 3; % Threshold for showing up on abstract picture
    end
    spikeskip = 10;
...
%     saveSpikes = sprintf('AbstractData_%d', naming);
% 
%           fid = fopen(saveSpikes,'a');
%           for x=1:simdur
%             if abstractGrid(x)>abstractThr
%                 fprintf(fid,'%d', 1);
%             else
%                 fprintf(fid, '%d', 0);
%             end
