function [ weights, partialcost, hidden2 ] = RBoltzmann( weights, visible, Nvisible, Nhidden)
            ...
            for i = 1:Nvisible
                for j = 1:(Nhidden+1)
                    if j == (Nhidden+1)
                        ...
                        deltaweights(i,j) = 0;                      
                    elseif (visible(i)*hidden(j) - hidden2(j)*reconstruction(i)) > 0
                        % Increment weights by a fixed step (g)
                        deltaweights(i,j) = g; % - g * weights(i,j);
                    elseif (visible(i)*hidden(j) - hidden2(j)*reconstruction(i)) < 0
                        % Decrement weights by a fixed step (g)
                        deltaweights(i,j) = -g; %  - g * weights(i,j);
                    else
                        deltaweights(i,j) = 0;
                    end
